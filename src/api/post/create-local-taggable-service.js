...
export async function getPermissions(dbs, req, res) {
    return {
        permissions: [PERMISSIONS.ADMINISTRATIVE.CREATE_LOCAL_TAGGABLE_SERVICE],
        objects: {}
    };
}
...
export default async function post(dbs, req, res) {
    await LocalTaggableServices.userInsert(dbs, req.user.id(), req.body.serviceName);
    res.status(200).send("Local taggable service created");
}
